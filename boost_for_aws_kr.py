...
train_data = std.fit_transform(X_train.iloc[:, impvars])
del(X_train)
#test_data = X_test.iloc[:,impvars]
test_data = std.transform(X_test.iloc[:,impvars])
del(X_test)
#train_submit = data_submit_dummy.iloc[:,impvars]
train_submit = std.transform(data_submit_dummy.iloc[:,impvars])


print("starting kr")
from sklearn.kernel_ridge import KernelRidge
kr = KernelRidge(alpha=1.0, coef0=1, degree=3, gamma=0.01, kernel='rbf',
      kernel_params=None)
...
boost_kr = en.BaggingRegressor(base_estimator = kr,
...
for i in range(1,no_steps+1):
	boost_kr.n_estimators = i * train_steps
	print('Step %i of %i. %i estimators' %(i,no_steps, i*train_steps))
	#t0 = time.time()
	boost_kr.fit(train_data, loss_train)
	...
	print(metrics.mean_absolute_error(loss_test, boost_kr.predict(test_data)))
	print("My error on train")
	print(metrics.mean_absolute_error(loss_train, boost_kr.predict(train_data)))
	# submit
	loss_submit = boost_kr.predict(train_submit)
	to_submit= pd.DataFrame({'id': submit_id, 'loss' : loss_submit})
	to_submit.to_csv('submitboostkr'+str(i*train_steps)+'estimators'+'.csv', index=False)
